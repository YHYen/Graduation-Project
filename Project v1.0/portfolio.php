<?php 
  ...
  if (!isset($_SESSION['is_login'])) {
    ...
    //已登入，取得id session, account session, password session
    $account = $_SESSION['account'];
    $password = $_SESSION['password']; 
    //連接資料庫
    require ("mysql_connect.inc.php");
    $sql_account_check = "SELECT * FROM record";
    $result = mysqli_query($db_link, $sql_account_check);
    //如果有讀到，就更新帳戶到member_email
    if ($result) {
      //取得當下資料筆數
      $num = mysqli_num_rows($result);
      //row[1] 是我要的資料
      $sql_data_onload = "UPDATE record SET member_email = '$account' WHERE Record_ID = '$num'";
      $result_Update = mysqli_query($db_link, $sql_data_onload) or die("失敗");
